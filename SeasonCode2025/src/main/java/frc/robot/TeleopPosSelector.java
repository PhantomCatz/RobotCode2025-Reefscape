...
public class TeleopPosSelector { //TODO split up the file. it's too big and does too much stuff
  ...
  public Command runToNetCommand(){
    return new InstantCommand(() -> {
      ...
      if(path != null){
        currentDrivetrainCommand = new TrajectoryDriveCmd(path, drivetrain, true, m_container, true, 0);
      }else{
  ...
  private void runLeftRightNBA(LeftRight leftRight) {
    ...
    currentDrivetrainCommand = new TrajectoryDriveCmd(path, drivetrain, true, m_container, true, superstructure.getLevel()).andThen(m_container.rumbleDrvAuxController(1.0, 0.2)).andThen(new InstantCommand(() -> isNBALeftRight = false));
  ...
  private void runLeftRightNet(LeftRight leftRight){
    ...
    currentDrivetrainCommand = new TrajectoryDriveCmd(path, drivetrain, false, m_container, true, 0);
    currentDrivetrainCommand.schedule();
  }
  ...
  public Command runToNearestBranch() {
    ...
    return new InstantCommand(() -> {
      ...
      try{
        ...
        currentDrivetrainCommand = new TrajectoryDriveCmd(path, m_container.getCatzDrivetrain(), true, m_container, true, superstructure.getLevel())
